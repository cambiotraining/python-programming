...
Before discussing python in more detail and trying to write any code, the best way of showing python's capabilities is to show what it can do, by running a script! Have a go at the first exercise in the exercises section. 

By the end of this course you will be able understand how this script works and be able to write the equivalent script yourself!
...
Q1. When looking at the script are there any keywords or symbols that you notice?

Q2. What logic is involved? - Can you create a flow chart reflecting what is done?

Q3. Now you have seen a small snapshot of what can be done with very basic python - can you think of any scripts you might want to write in Python?

::: {.callout-answer}

Q1. you might notice the use of ```"x"``` quotation marks, ```=``` equals sign, ```{}``` curly brackets, and ```:``` colons.

By the end of this course you will be able to produce a script like this one.
...
# # Initialize plot figure and canvas bounds
...
# # Flowchart Node Definition: [x, y, label, node_type]
# nodes = [
#     (5, 11.0, "Start Script", "terminal"),
#     (
#         5,
#         9.5,
...
#         5,
#         7.8,
#         "2. Load Expenses\nfile_reader.load_expenses(FILE_NAME)\n[Try / Except]",
#         "process",
#     ),
#     (
#         5,
#         6.0,
#         "3. Collect Expenses Loop\ninput_handler.collect_expenses(...)\n[While Loop]",
#         "process",
#     ),
#     (
#         5,
...
#         5,
#         2.4,
#         "5. Save Expenses\nfile_writer.save_expenses(...)\n['With' Statement]",
#         "process",
#     ),
#     (5, 0.9, "End Script", "terminal"),
# ]

# # Side annotations showing external module dependencies
# module_helpers = [
#     (8, 7.8, "file_reader"),
#     (8, 6.0, "input_handler\nbasics, logic"),
#     (8, 4.2, "summary"),
...
#     # Adjust vertical offset based on block type
#     offset_top = 0.5 if nodes[i][3] == "process" else 0.3
#     offset_bottom = 0.5 if nodes[i + 1][3] == "process" else 0.3

#     ax.annotate(
#         "",
#         xy=(x2, y2 + offset_bottom),
#         xytext=(x1, y1 - offset_top),
#         arrowprops=dict(
#             arrowstyle="->", color="#334155", lw=2, mutation_scale=15
#         ),
#     )
...
#     else:
#         bbox = dict(
#             boxstyle="round,pad=0.7,rounding_size=0.2",
#             facecolor="#FFFFFF",
#             edgecolor="#64748B",
#             linewidth=1.5,
#         )
...
# # Draw Side Module Badges & Dashed Connectors
# for x, y, label in module_helpers:
#     ax.text(
#         x,
#         y,
#         f"Module: {label}",
...
#     # Dashed connector from main node to module box
#     ax.plot([6.5, x - 0.9], [y, y], linestyle="--", color="#94A3B8", linewidth=1)

# # Polish Axis and Title Layout
# ax.axis("off")
# plt.title(
#     "Expense Script Execution Flow",
#     fontsize=14,
#     fontweight="bold",
#     pad=15,
#     color="#1E293B",
...
# 1. Setup the figure and canvas dimensions
...
# 2. Define beginner-friendly steps [x, y, simple_title, simple_explanation, block_type]
steps = [
    (5, 9.2, "START", "Begin Program", "terminal"),
    (
        5,
        7.8,
        "Step 1: Set Settings",
        "Save file name and budget limits",
        "process",
    ),
    (
        5,
        6.3,
        "Step 2: Read Expenses",
    ...
    (
        5,
        4.8,
        "Step 3: Collect New Input",
        "Repeat Task: Ask for user expenses",
        "process",
    ),
    (
        5,
        3.3,
        "Step 4: Show Summary",
        "Go item-by-item through total list",
        "process",
    ),
    (
        5,
        1.8,
        "Step 5: Save File",
        "Store updated expenses safely",
        "process",
    ),
    (5, 0.6, "END", "Finish Program", "terminal"),
]

# 3. Draw simple straight connecting arrows
for i in range(len(steps) - 1):
    y_start = steps[i][1]
    y_end = steps[i + 1][1]

    # Adjust vertical arrow offset based on box height
    offset_top = 0.35 if steps[i][4] == "terminal" else 0.5
    offset_bottom = 0.35 if steps[i + 1][4] == "terminal" else 0.5

    ax.annotate(
        "",
        xy=(5, y_end + offset_bottom),
        xytext=(5, y_start - offset_top),
        arrowprops=dict(
            arrowstyle="->", color="#475569", lw=2, mutation_scale=15
        ),
    )

# 4. Draw the flowchart boxes
for x, y, title, description, block_type in steps:
    ...
    ax.text(
        ...
        va="center",
        fontsize=10,
        fontweight="bold" if block_type == "terminal" else "normal",
        color=text_color,
        bbox=box_style,
        linespacing=1.4,
    )

# 5. Clean up axes and show chart
ax.axis("off")
plt.title(
    "How the Expense Script Works",
    fontsize=14,
    fontweight="bold",
    pad=15,
    color="#0F172A",
